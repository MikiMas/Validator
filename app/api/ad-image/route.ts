...
// Next.js route handlers are discovered as `route.ts` / `route.js` (not `.tsx`).
...
export async function GET(request: Request) {
  ...
  const message = clampText(searchParams.get("message") ?? "Tu mensaje aquí", 300);
  ...
  const root = React.createElement(
    ...
    React.createElement(
      "div",
      { style: { marginTop: "auto", display: "flex", alignItems: "center", gap: 18 } },
      ...
      React.createElement(
        "div",
        { style: { color: "rgba(255,255,255,0.92)", fontWeight: 650, fontSize: 34 } },
        brand
      )
    )
  );
